class User < ApplicationRecord
  ...
  has_many :emails, -> {where(recipients: {status: 1})}, through: :recipients
  ...
  has_many :trash_received, -> {where(recipients: {status: 0})}, through: :recipients, source: 'email'
  has_secure_password
  ...
  def trash_emails
    self.trash_received.merge(self.trash_sent)
  end
end
